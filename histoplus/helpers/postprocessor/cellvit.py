...
class CellViTPostprocessor(Postprocessor):
    ...
    def _process_np_hv_maps(
        ...
        # 권장: 먼저 작은 잡음 제거를 약하게 하고, 근접 seed 병합을 유도
        kernel3 = cv2.getStructuringElement(cv2.MORPH_ELLIPSE, (3,3))
        kernel5 = cv2.getStructuringElement(cv2.MORPH_ELLIPSE, (5,5))
        marker = cv2.morphologyEx(marker, cv2.MORPH_OPEN, kernel3)   # 약하게 노이즈 제거
        marker = cv2.morphologyEx(marker, cv2.MORPH_CLOSE, kernel5)  # 경계 틈 메워 seed 병합
        # (선택) 아주 인접 seed를 합치고 싶으면 한 번만 팽창
        marker = cv2.dilate(marker, kernel3, iterations=1)

        marker = label(marker)[0]
        marker = remove_small_objects(marker, min_size=100)
